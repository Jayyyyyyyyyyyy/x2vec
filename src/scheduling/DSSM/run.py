...
def run_task(args):
    ...
    try:
        ...
        sched.add_task("build_sample", "{} run_build_sample.py --date {} --conf {} --expire".format(
            py_bin, args.date, args.conf
        ), True)
        sched.add_task("build_train_tfrecords", "{} run_build_tfrecords.py --date {} --conf {} --stage train --expire".format(
            py_bin, args.date, args.conf
        ), True)
        sched.add_task("build_test_tfrecords", "{} run_build_tfrecords.py --date {} --conf {} --stage test --expire".format(
            py_bin, args.date, args.conf
        ), True)
        sched.add_task("build_inference", "{} run_build_inference.py --date {} --conf {} --expire".format(
            py_bin, args.date, args.conf
        ), True)
        sched.add_task("build_inference_tfrecords", "{} run_build_tfrecords.py --date {} --conf {} --stage inference --expire".format(
            py_bin, args.date, args.conf
        ), True)
        sched.add_task("train", "{} run_train.py --date {} --conf {}".format(
            py_bin, args.date, args.conf
        ), True)
        sched.add_task("inference", "{} run_inference.py --date {} --conf {}".format(
            py_bin, args.date, args.conf
        ), True)
        ...
        sched.set_dependency("build_sample", "build_train_tfrecords")
        sched.set_dependency("build_train_tfrecords", "build_test_tfrecords")
        sched.set_dependency("build_sample", "build_inference")
        sched.set_dependency("build_inference", "build_inference_tfrecords")
        sched.set_dependency("build_train_tfrecords", "train")
        sched.set_dependency("build_test_tfrecords", "train")
        sched.set_dependency("train", "inference")
        sched.set_dependency("build_inference_tfrecords", "inference")
