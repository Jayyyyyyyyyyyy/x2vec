...
def process_build_samples(args):
    ...
    output_path = config.get("common", "hdfs_output", 0, {
        "date": args.date,
        "dir": config.get("inference_sample", "raw_dir")
    })

    if hadoop_shell.exists_all(output_path, flag=True, print_missing=True):
        logging.error("output already exists, skip")
        return 0
    ...
    ss.set_master("yarn")\
        ...
        .set_name(config.get("common", "job_name", 0, {'date': args.date, 'module': "BuildInference"}))\
        .set_queue(config.get("common", "job_queue"))\
        .set_class("com.td.ml.x2vec.DSSM.BuildInference")\
...
def del_expire(args):
    ...
    lifetime = config.getint("inference_sample", "lifetime")
